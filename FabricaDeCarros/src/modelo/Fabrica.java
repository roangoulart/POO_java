	...
	public void venderCarro(String modelo, String cor) {
		...
		for (int i = 0; i < carrosFab.size(); i++) {
			...
			if ((c.getModelo().equals(modelo)) && (c.getCor().equals(cor))) {
				carrosFab.remove(c);
				JOptionPane.showMessageDialog(null, "Veículo vendido!", "Vendido", JOptionPane.INFORMATION_MESSAGE);
			} else {
				JOptionPane.showMessageDialog(null, "Veículo não encontrado!", "Erro", JOptionPane.WARNING_MESSAGE);
			}
		}
	}
}
